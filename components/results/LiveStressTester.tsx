...
function Slider({
  label, value, min, max, step, format, onChange, color = 'amber',
}: {
  ...
  const trackColor: Record<string, string> = {
    amber: '#f59e0b', blue: '#3b82f6', green: '#22c55e', purple: '#8b5cf6',
  };
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-gray-700">{label}</span>
        <span className="text-sm font-bold tabular-nums" style={{ color: trackColor[color] }}>
          {format(value)}
        </span>
      </div>
      <div className="relative h-5 flex items-center">
        <div className="w-full h-1.5 rounded-full bg-gray-200">
          <div className="h-1.5 rounded-full" style={{ width: `${pct}%`, backgroundColor: trackColor[color] }} />
        </div>
        <input
...
export function LiveStressTester({
  ...
  const metrics = useMemo(() => {
    const scaledSavings = (year1SolarSavings + year1ExportIncome + year1BatteryValue + year1EvSavings) * energyMult * panelMult;
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-gray-600 leading-relaxed">{t.stressTester.intro}</p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 bg-gray-50 rounded-2xl p-5">
        <Slider
          label={t.stressTester.equityLabel}
          value={equityPct} min={0} max={1} step={0.05}
          format={(v) => `${Math.round(v * 100)}%`}
          onChange={setEquityPct} color="amber"
        />
        <Slider
          label={t.stressTester.rateLabel}
          value={annualRate} min={0.02} max={0.18} step={0.005}
          format={(v) => `${(v * 100).toFixed(1)}%`}
          onChange={setAnnualRate} color="blue"
        />
        <Slider
          label={t.stressTester.energyLabel}
          value={energyMult} min={0.5} max={2.0} step={0.05}
          format={(v) => v === 1 ? t.stressTester.base : v < 1 ? `−${Math.round((1 - v) * 100)}%` : `+${Math.round((v - 1) * 100)}%`}
          onChange={setEnergyMult} color="green"
        />
        <Slider
          label={t.stressTester.sizeLabel}
          value={panelMult} min={0.5} max={2.0} step={0.05}
          format={(v) => v === 1 ? t.stressTester.asDesigned : v < 1 ? `−${Math.round((1 - v) * 100)}%` : `+${Math.round((v - 1) * 100)}%`}
          onChange={setPanelMult} color="purple"
        />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        />
        <MetricPill
          label={t.stressTester.irrLabel}
          value={irrVal != null ? `${(irrVal * 100).toFixed(1)}%` : equityPct < 0.01 ? t.stressTester.noEquity : t.common.na}
          color={irrVal != null ? (irrVal > 0.12 ? 'green' : irrVal > 0.07 ? 'yellow' : 'red') : 'gray'}
        />
        <MetricPill
          label={t.stressTester.npvLabel}
          value={`${symbol}${Math.round(npvVal).toLocaleString()}`}
          color={npvVal >= 0 ? 'green' : 'red'}
        />
        <MetricPill
          label={fmt(t.stressTester.lifetimeLabel, { n: horizonYears })}
          value={`${symbol}${Math.round(lifetimeVal).toLocaleString()}`}
          color={lifetimeVal > 0 ? 'green' : 'red'}
        />
        <MetricPill
          label={t.stressTester.year1Label}
      ...
      {isModified && (
        <div className="flex justify-end">
          <button onClick={reset} className="text-xs text-gray-500 hover:text-gray-900 underline underline-offset-2">
            {t.stressTester.resetBtn}
          </button>
        </div>
      )}

      <p className="text-[11px] text-gray-400 leading-relaxed">
        {fmt(t.stressTester.disclaimer, { n: inverterReplacementYear })}
      </p>
    </div>
  );
}
